...
export const CompanyDetails: React.FC = () => {
	...
	const handleSubmitCompanyData: SubmitHandler<CompanyDataForm> = async (data) => {
		setDisplayErrors('')
	...
	const handleSubmitManagerData: SubmitHandler<ManagerDataForm> = async (data) => {
		setDisplayErrors('')
		...
		try {
			...
			enqueueSnackbar('Dados do gestor atualizados com sucesso!', { variant: 'success' })
		} catch (err) {
			enqueueApiError(err)
			return
		}
	...
	const handleChangeAuditorsSave = async () => {
		try {
			...
			enqueueSnackbar('Auditores atualizados com sucesso!', { variant: 'success' })
		} catch (err: any) {
			enqueueSnackbar(err.response.data.message, { variant: 'error' })
		}
			...
			<Body data-testid="company-details-body">
					...
					}
					{
						...
						<li className="manager-data" data-testid="tab-manager-form">
							<Form className='add-manager-data-form' onSubmit={handleSubmitManagerData} ref={formManagerRef}>
								...
								<Input label='Telefone' name='phone' onChange={e => handlePhoneChange(e.target.value, formManagerRef, 'phone')} />
								<Alert
									text={displayErrors}
									type='error'
									testid="display-errors"
				...
				<Dialog open={auditorsDialogOpen} toggleOpen={toggleAuditorsDialogOpen}>
					<AuditorsDialogContent>
						<CloseIcon className='close-dialog-icon' onClick={toggleAuditorsDialogOpen} data-testid="close-button" />
						...
						<div className="dialog-body">
							...
							<Button buttonStyle={BT_PRIMARY} text='Concluído' className='bt-auditors-finished' onClick={handleChangeAuditorsSave} />
						</div>
					</AuditorsDialogContent>
				</Dialog>
			</Body>
