...
export const AddEmployeeToQuestionaryDialog: React.FC<
	AddEmployeeToQuestionaryDialogProps
> = ({ isOpen, toggleOpen, questionaryId }) => {
		...
		;(async () => {
			try {
				const { data } = await api.get(
					`/employees/available-to-questionary/${questionaryId}`,
				)
	...
	useEffect(() => {
		...
		;(async () => {
			try {
				const { data } = await api.get(
					`/employees/current-in-questionary/${questionaryId}`,
				)
	...
	const handleRemoveEmployee = async (employeeId: string) => {
		try {
			await api.delete(
				`/questionaries/${questionaryId}/employees/${employeeId}`,
			)
	...
	const handleSaveEmployeesToQuestionaryButtonClick = async () => {
		try {
			await api.post(`/questionaries/${questionaryId}/employees`, {
				employees: selectedEmployees,
			})
			enqueueSnackbar(
				'Os usuário que tem permissão para responder este questionário foram atualizados!',
				{
					variant: 'success',
				},
			)
		...
		<Dialog open={isOpen} toggleOpen={toggleDialogOpen}>
			<DialogContainer>
				...
				<div className="dialog-header">
					<AddIcon />
					<h2>Adicionar usuários ao questionário</h2>
				</div>
