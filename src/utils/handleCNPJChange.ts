...
export const handleCNPJChange = (value: string, formRef: React.RefObject<FormHandles>, fieldName: string) => {
	if (!value)
		return
	...
	formRef.current?.setFieldValue(fieldName, formattedCNPJValue.replace(/(\d{2})(\d{3})(\d{3})(\d{4})(\d{2})/, '$1.$2.$3/$4-$5')
	)
}
