...
describe('<CompanyDetails/>', () => {
	beforeEach(async () => {
		mockAxios.get
			.mockImplementation()
			.mockResolvedValue({ data: { company, availableAuditors } })
	...
	it('should be able to render the page correctly', async () => {
		...
		expect(
			await companyDetailsPageElement.findByText('Empresas'),
		).toBeInTheDocument()
		expect(
			await companyDetailsPageElement.findByText('Auditores'),
		).toBeInTheDocument()
		expect(
			await companyDetailsPageElement.findByTestId('back-button'),
		).toBeInTheDocument()
		expect(
			await companyDetailsPageElement.findByText('Dados da empresa'),
		).toBeInTheDocument()
		expect(
			await companyDetailsPageElement.findByText('Dados do gestor'),
		).toBeInTheDocument()
		expect(
			await companyDetailsPageElement.findByLabelText('Nome da empresa'),
		).toBeInTheDocument()
		expect(
			await companyDetailsPageElement.findByLabelText('CNPJ'),
		).toBeInTheDocument()
		expect(
			await companyDetailsPageElement.findByLabelText('Site'),
		).toBeInTheDocument()
		expect(
			(
				await companyDetailsPageElement.findByLabelText('Nome da empresa')
		...
		expect(
			(await companyDetailsPageElement.findByLabelText('CNPJ')).closest('input')
				?.value,
		).toBe(company.cnpj)
		expect(
			(await companyDetailsPageElement.findByLabelText('Site')).closest('input')
				?.value,
		).toBe(company.site)
		expect(
			await companyDetailsPageElement.findByText('Salvar alterações'),
		).toBeInTheDocument()
	})

	it('should be able to go back to companies list if user click in back button', async () => {
		const backButton = await companyDetailsPageElement.findByTestId(
			'back-button',
		)
		await act(async () => {
	...
	it('should be able to change tabs and initial data should show in all cases', async () => {
		const managerDataTabButton = await companyDetailsPageElement.findByText(
			'Dados do gestor',
		)
		await act(async () => {
		...
		expect(
			companyDetailsPageElement.queryByTestId('tab-company-form'),
		).not.toBeInTheDocument()
		expect(
			companyDetailsPageElement.queryByTestId('tab-manager-form'),
		).toBeInTheDocument()
		expect(
			(await companyDetailsPageElement.findByLabelText('Nome')).closest('input')
				?.value,
		).toBe(company.manager?.name)
		expect(
			(await companyDetailsPageElement.findByLabelText('Cargo')).closest(
				'input',
			)?.value,
		).toBe(company.manager?.office)

		const companyDataTabButton = await companyDetailsPageElement.findByText(
			'Dados da empresa',
		)
		await act(async () => {
	...
	describe('form validations', () => {
		describe('company data form', () => {
			it('should not be able to save company data without the company name', async () => {
				...
				const saveButton = await companyDetailsPageElement.findByText(
					'Salvar alterações',
				)
				await act(async () => {
				...
				const errorMessage = await companyDetailsPageElement.findByTestId(
					'display-errors',
				)
		...
		describe('manager data form', () => {
			...
			it('should not be able to save data if name is not filled', async () => {
				const managerNameInput =
					await companyDetailsPageElement.findByLabelText('Nome')
			...
			it('should not be able to save data if office is not filled', async () => {
				const managerOfficeInput =
					await companyDetailsPageElement.findByLabelText('Cargo')
			...
			it('should not be able to save data if department is not filled', async () => {
				const managerDepartmentInput =
					await companyDetailsPageElement.findByLabelText('Departamento')
			...
			it('should not be able to save data if email is not filled', async () => {
				const managerEmailInput =
					await companyDetailsPageElement.findByLabelText('Email')
			...
			it('should not be able to save data if phone is unfilled', async () => {
				const managerPhoneInput =
					await companyDetailsPageElement.findByLabelText('Telefone')
				...
				expect(errorMessage).toHaveTextContent(
					'Por favor, preencha o telefone deste gestor.',
				)
			})
	...
	describe('auditors dialog', () => {
		beforeEach(async () => {
			const auditorsOpenDialogButton =
				await companyDetailsPageElement.findByText('Auditores')
			await act(async () => {
		...
		it('should be able to open and render the auditors dialog correctly', async () => {
			expect(
				await companyDetailsPageElement.findByText('Pessoas com acesso'),
			).toBeVisible()
			expect(
				await companyDetailsPageElement.findByLabelText('Adicione auditores'),
			).toBeVisible()
			expect(
				await companyDetailsPageElement.findByText('Concluído'),
			).toBeVisible()
		})

		it('should close the auditors dialog when click in Concluído or in close button', async () => {
			const closeButton = await companyDetailsPageElement.findByTestId(
				'close-button',
			)
			await act(async () => {
			...
			expect(
				companyDetailsPageElement.queryByText('Pessoas com acesso'),
			).not.toBeVisible()
			...
			const concluedButton = await companyDetailsPageElement.findByText(
				'Concluído',
			)
			await act(async () => {
		...
		it('should be able to show all auditors when user click in find for input, allow select and save to database', async () => {
			const auditorsSelect = await companyDetailsPageElement.findByTestId(
				'auditors-select',
			)
			const auditorSelectInput = within(auditorsSelect).getByRole('combobox')
			auditorsSelect.focus()

			fireEvent.change(auditorSelectInput, {
				target: { value: 'Renato Silva' },
			})
		...
		it('should be able to show all registered auditors', async () => {
			expect(
				await companyDetailsPageElement.findByText('Patrick Pessoa'),
			).toBeInTheDocument()
			expect(
				await companyDetailsPageElement.findByText('Raquel Ribeiro'),
			).toBeInTheDocument()
		})
	})
})
