...
describe('<Companies />', () => {
	...
	it('should be able to render correctly the page', async () => {
		...
		expect(await companiesPageElement.findAllByTestId('company-card')).toHaveLength(3)
	})
	...
	it('should be able to go to Create first company page if no companies has found in database', async () => {
		...
		expect(companiesPageElement.queryAllByTestId('company-card')).toHaveLength(0)
		expect(companiesPageElement.queryByTestId('company-card')).not.toBeInTheDocument()
		expect(mockedUseNavigate).toHaveBeenCalledTimes(1)
		expect(mockedUseNavigate).toHaveBeenCalledWith('/cadastre-sua-primeira-empresa')
	})
	...
	it('should be able to see auditors list when click in auditors avatar', async () => {
		...
		const firstCompanyCardAuditors = (await companiesPageElement.findAllByTestId('auditors'))[0]

		await act(async () => {
			fireEvent.click(firstCompanyCardAuditors)
		})

		expect(await companiesPageElement.findByText(companies[0].auditors[0].name)).toBeInTheDocument()
	})
	...
	it('should be able to open the Create new company drawer when click in "Criar nova empresa +"', async () => {
		...
		const buttonCreateNewCompany = await companiesPageElement.findByText('Criar nova empresa +')
		await act(async () => {
			fireEvent.click(buttonCreateNewCompany)
		})

		expect(companiesPageElement.queryByTestId('create-company-drawer')).toBeInTheDocument()
	})
	...
	it('should be able to open the company data when click in company name in the card title', async () => {
		...
		const companyCardTitle = await companiesPageElement.findByText(companies[0].name)

		fireEvent.click(companyCardTitle)

		expect(mockedUseNavigate).toHaveBeenCalledTimes(1)
		expect(mockedUseNavigate).toHaveBeenCalledWith(`/detalhes-da-empresa/${companies[0]._eq}`)
	})
	...
	describe('create new company', () => {
		it('should be able to close the drawer when clicked in X on left top', async () => {
			...
			const closeDrawerButton = await companiesPageElement.findByTestId('close-drawer-button')
			await act(async () => {
		...
		it('should be able to close drawer when save company without manager data', async () => {
			...
			const companyNameInput = await companiesPageElement.findByLabelText('Nome da empresa')
			const companyCNPJInput = await companiesPageElement.findByLabelText('CNPJ')
			const companySiteInput = await companiesPageElement.findByLabelText('Site')
			...
			const jumpStepButton = await companiesPageElement.findByText('Pular etapa')
		...
		it('should be able to close drawer when save company with manager data', async () => {
			...
			const registerCompanyButton = await companiesPageElement.findByText('Cadastrar empresa')
			...
			const departmentInput = await companiesPageElement.findByLabelText('Departamento')
